...
export function ProjectCard({ project, dimmed }: ProjectCardProps) {
  return (
    <article
    ...
    >
      ...
      <header className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-neon-400">{project.client}</p>
            <h3 className="text-xl font-semibold text-ink-25">{project.role}</h3>
          </div>
        </div>

        <p className="font-mono text-xs uppercase tracking-[0.16em] text-ink-100">{project.period}</p>
      </header>
